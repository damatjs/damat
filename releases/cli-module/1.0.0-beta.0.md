...
Module commands now use the shared transactional installer for add,
...
New module scaffolds write root `damat.json`; legacy
...
The vendor-specific `damat auth init` command group was removed before beta.
Auth providers install and update through the ordinary `damat module` commands;
integration notices instruct owners to configure the module and its provider
role binding.
