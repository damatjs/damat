...
|---------|---------|---------------|
| Unreleased | `damat build` type-checks the whole app before bundling (fails on any error; `--no-typecheck` opts out) and fails on a broken config bundle; new `damat module build` gate (type-check + contract validate). | [Unreleased →](./next.md) |
