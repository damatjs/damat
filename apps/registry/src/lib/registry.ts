...
function normalize(key: string, entry: RegistryModuleEntry): Module {
  ...
  const versions: NormalizedVersion[] = Object.entries(entry.versions ?? {})
    .map(([version, v]) => ({ version, source: typeof v === 'string' ? v : v.source }))
    .sort((a, b) => compareVersionsDesc(a.version, b.version))

  const status = entry.verification?.status ?? (entry.owner?.verified ? 'verified' : 'unverified')
  ...
  return {
    ...
    status,
    ...(entry.verification?.reason ? { reason: entry.verification.reason } : {}),
    ...(entry.verification?.verifiedBy ? { verifiedBy: entry.verification.verifiedBy } : {}),
