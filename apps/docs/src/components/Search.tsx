...
import { CornerDownLeftIcon, HashIcon, SearchIcon } from './icons'
...
export function SearchDialog({
  ...
  return (
    ...
    >
      ...
      />
      <div
        className="relative w-full max-w-xl overflow-hidden rounded-2xl border border-line bg-surface shadow-2xl animate-fade-in"
        onKeyDown={onKeyDown}
      >
        <div className="flex items-center gap-3 border-b border-line px-4">
          ...
          <input
            ...
            className="w-full bg-transparent py-4 text-[0.95rem] text-ink outline-none placeholder:text-faint"
          />
          <kbd className="hidden shrink-0 rounded border border-line px-1.5 py-0.5 text-[0.65rem] text-faint sm:block">
            ESC
          </kbd>
        </div>
          ...
          {hits.length === 0 ? (
            ...
            hits.map((hit, i) => (
              ...
              >
                ...
                {i === active && (
                  <CornerDownLeftIcon width={14} height={14} className="mt-1.5 shrink-0 text-faint" />
                )}
              </button>
            ))
          )}
