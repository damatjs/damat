...
export function CodeEnhancer() {
  ...
  // biome-ignore lint/correctness/useExhaustiveDependencies: pathname intentionally re-enhances the new page after navigation
  useEffect(() => {
    const container = document.querySelector("[data-doc-content]");
    if (!container) return;

    const blocks = Array.from(
      container.querySelectorAll<HTMLPreElement>("pre.shiki"),
    );
