...
1. **Start the module** — two ways, same resulting shape:
   - `bunx @damatjs/damat-cli@latest module init <name>` — the getting-started command.
     It scaffolds locally, so you get the same deterministic output without
     depending on a remote starter repo.
