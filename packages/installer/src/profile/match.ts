...
export function matchProfiles(input: MatchProfilesInput): CapabilityMatch[] {
  ...
  const unknownOverride = Object.keys(overrides).find((name) => !provides[name]);
  if (unknownOverride)
    throw new Error(`target override has no provided capability: ${unknownOverride}`);
  return Object.keys(provides).sort().map((capability) => {
    const provided = provides[capability]!;
    const destination = overrides[capability] ?? accepts[capability]?.to
      ?? provided.fallbackTo;
    if (!destination)
