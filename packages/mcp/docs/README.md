...
| `src/registry/`         | Registry layer — ref parsing, index fetch/load/cache, unique bare-ref lookup, verdict loading, and entry summaries.                                                         |
...
| `module_info`    | `ref`                                       | Exact ref lookup, then one unique key/metadata match for a bare name; ambiguous namespaces are reported.   |
