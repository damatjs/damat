...
describe.skipIf(!process.env.DATABASE_URL)(
  ...
  () => {
    ...
    beforeAll(async () => {
      ...
      await pool.query(
        `DROP TABLE IF EXISTS member_accounts, associations CASCADE`,
      );
      await pool.query(
        `CREATE TABLE associations (id text primary key, name text)`,
      );
      await pool.query(
    ...
    it("cascade-deletes the snake_case child with the parent", async () => {
      await pool.query(
        `INSERT INTO associations (id, name) VALUES ('a1', 'HOA')`,
      );
      await pool.query(
      ...
      const deleted = await assoc.delete({
        where: { id: "a1" },
        cascade: true,
      });
