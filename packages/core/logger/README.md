...
getLogger().request({ requestId: "req_9", method: "GET", path: "/health", status: 200, duration: 4 });
```
...
| `close()` *(Logger only)*           | Flush & close the file transport.                            |

### Key types (`types.ts`)

```ts
type LogLevel =
  | "debug" | "info" | "progress" | "waiting" | "cached"
