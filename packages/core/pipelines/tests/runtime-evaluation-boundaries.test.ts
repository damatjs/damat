...
import { evaluatePipelineExpression, evaluatePipelineValue } from "../src";
...
test("pipeline values preserve primitives, arrays, objects, and missing values", () => {
  ...
  expect(evaluatePipelineValue([1, { $ref: "input.count" }], context)).toEqual([
    1, 2,
  ]);
...
test("pipeline expressions cover every logical and comparison operator", () => {
  ...
  expect(
    evaluatePipelineExpression({ op: "exists", value: ref("count") }, context),
  ).toBe(true);
  expect(
    evaluatePipelineExpression(
      { op: "exists", value: ref("missing") },
      context,
    ),
  ).toBe(false);
  expect(
  ...
  expect(
    evaluatePipelineExpression(
      { op: "and", values: [{ op: "eq", left: 1, right: 1 }] },
      context,
    ),
  ).toBe(true);
  expect(
  ...
  expect(
    evaluatePipelineExpression({ op: "neq", left: 1, right: 2 }, context),
  ).toBe(true);
  expect(
    evaluatePipelineExpression(
      { op: "gt", left: ref("count"), right: 1 },
      context,
    ),
  ).toBe(true);
  expect(
    evaluatePipelineExpression({ op: "gte", left: 2, right: 2 }, context),
  ).toBe(true);
  expect(
    evaluatePipelineExpression(
      { op: "lt", left: "a", right: ref("text") },
      context,
    ),
  ).toBe(true);
  expect(
    evaluatePipelineExpression({ op: "lte", left: "b", right: "b" }, context),
  ).toBe(true);
  expect(
    evaluatePipelineExpression(
      { op: "in", left: 2, right: ref("list") },
      context,
    ),
  ).toBe(true);
  expect(
    evaluatePipelineExpression({ op: "in", left: 2, right: 2 }, context),
  ).toBe(false);
  expect(
    evaluatePipelineExpression({ op: "gt", left: null, right: 1 }, context),
  ).toBe(false);
  expect(evaluatePipelineExpression({ op: "invalid" } as never, context)).toBe(
    false,
  );
});
