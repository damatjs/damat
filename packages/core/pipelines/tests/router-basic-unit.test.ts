...
test("generic control processing parks join nodes", async () => {
  ...
  const executor = { query: async (sql: string) => {
    queries.push(sql);
    return { rows: [], rowCount: 1 };
  } };
  await processControlNode(executor as never, {} as never, execution, { id: "node", kind: "join" });
...
test("waiting an already parked node without a deadline is a no-op", async () => {
  let called = false;
  await waitNode({ query: async () => { called = true; } } as never, {
    ...execution,
    status: "waiting",
  } as never);
...
test("pipeline errors serialize Error instances and arbitrary failures", () => {
  expect(serializePipelineError(new TypeError("bad"))).toEqual({ name: "TypeError", message: "bad" });
  expect(serializePipelineError("bad")).toEqual({ name: "Error", message: "bad" });
});
...
test("terminal projection rejects nodes absent from the pinned manifest", async () => {
  ...
  await expect(projectTerminalJob(executor as never, {
    run_id: "run",
    node_id: "missing",
    phase: "forward",
    job_status: "succeeded",
    job_error: null,
  } as never)).rejects.toThrow("missing from its pinned manifest");
});
