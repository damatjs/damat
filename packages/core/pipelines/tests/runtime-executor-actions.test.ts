...
test("pipeline executor registers once and preserves its queue", () => {
  registerPipelineExecutorJob();
  expect(getJobDefinition(PIPELINE_EXECUTOR_JOB)?.options.queue).toBe(
    "damat-pipelines",
  );
  expect(() => registerPipelineExecutorJob()).not.toThrow();
  expect(() => registerPipelineExecutorJob("other-pipeline-queue")).toThrow(
    "queue changed",
  );
});
...
test("pipeline executor validates and invokes action capabilities", async () => {
  ...
  definePipelineAction({
    ...
    handler: (input, actionContext) => {
      received = [
        input,
        actionContext.pipelineRunId,
        actionContext.nodeExecutionId,
  ...
  await expect(handler()(payload(name, "bad"), context)).rejects.toThrow(
    `${name}.input`,
  );
  await expect(
    handler()(payload(`missing-${crypto.randomUUID()}`, 1), context),
  ).rejects.toThrow("Unknown pipeline action");
});
...
test("pipeline executor validates action output schemas", async () => {
  ...
  await expect(handler()(payload(name, null), context)).rejects.toThrow(
    `${name}.output`,
  );
});
