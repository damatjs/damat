...
const child = () =>
  definePipeline(uniqueName("loop-child"), {
    version: 1,
    start: "wait",
    nodes: [{ id: "wait", kind: "delay" as const, delayMs: 0 }],
    edges: [],
  });
const loop = (pipeline: string, until: never, maxIterations: number) =>
  definePipeline(uniqueName("loop-parent"), {
    version: 1,
    start: "loop",
...
test("loop nodes complete, iterate, and enforce their configured bound", async () => {
  const nested = child();
  const immediate = loop(
    nested.name,
    { op: "exists", value: { $ref: "item" } } as never,
    2,
  );
  const repeated = loop(
    nested.name,
    { op: "gte", left: { $ref: "iteration" }, right: 2 } as never,
    2,
  );
  const limited = loop(
    nested.name,
    { op: "eq", left: 1, right: 2 } as never,
    1,
  );
  await syncPipelineDefinitions();
  expect(
    (await routeToTerminal((await startPipeline(immediate.name, {})).id))
      .status,
  ).toBe("succeeded");
  expect(
    (await routeToTerminal((await startPipeline(repeated.name, {})).id)).status,
  ).toBe("succeeded");
  expect(
    (await routeToTerminal((await startPipeline(limited.name, {})).id)).status,
  ).toBe("failed");
});
