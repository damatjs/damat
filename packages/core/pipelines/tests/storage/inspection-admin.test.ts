...
test("active runs can be paused, resumed, and cancelled", async () => {
  ...
  await expect(client().pause(run.id, control("pause-again"))).rejects.toThrow(
    "cannot be paused",
  );
  await client().resume(run.id, control("resume"));
  expect((await findPipelineRun(run.id))?.status).toBe("running");
  await expect(
    client().resume(run.id, control("resume-again")),
  ...
  expect((await listPipelineNodeExecutions(run.id))[0]?.status).toBe(
    "cancelled",
  );
});
...
test("failed nodes without downstream work can be retried", async () => {
  ...
  expect((await findPipelineRun(run.id))?.status).toBe("running");
  await expect(
    client().retryNode(run.id, node.id, control("retry-invalid")),
  ).rejects.toThrow("failed or cancelled");
});

test("administrative controls validate targets and audit information", async () => {
  const missing = crypto.randomUUID();
  await expect(client().pause(missing, control("missing"))).rejects.toThrow(
    "was not found",
  );
  await expect(
    client().cancel(missing, control("cancel-missing")),
  ).rejects.toThrow("was not found");
  await expect(client().pause(missing, control("reason", ""))).rejects.toThrow(
    "reason is required",
  );
  await expect(client().pause(missing, control("", "reason"))).rejects.toThrow(
    "idempotency key",
