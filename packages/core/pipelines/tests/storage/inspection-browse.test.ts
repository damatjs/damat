...
test("inspection lists cursor pages and filters canonical runs", async () => {
  ...
  const client = createPipelineInspectionClient({ cursorSigningKey: key, client: durability });
  const page = await client.listRuns({ limit: 1 });
  expect(page.items).toHaveLength(1);
  expect(page.nextCursor).toBeString();
  expect((await client.listRuns({ limit: 1, cursor: page.nextCursor })).items).toHaveLength(1);
  expect((await client.listRuns({ name: second.name, status: "running", limit: 999 })).items[0]?.id).toBe(second.id);
  expect((await client.listRuns({ name: first.name, limit: 0 })).items).toHaveLength(1);
});
...
test("detail snapshots include graph records and respect visibility", async () => {
  ...
  await pool.query(
    ...
    [crypto.randomUUID(), started.id, JSON.stringify({ secret: "signal" }), JSON.stringify({ id: "actor" })],
  );
  const authoring = createPipelineAuthoringClient({ client: durability });
  await authoring.saveLayout(completed.versionId, { x: 1 }, {
    actor: { id: "editor", type: "user" }, reason: "inspection layout", idempotencyKey: "layout",
  });
  const full = createPipelineInspectionClient({
    cursorSigningKey: key, visibility: "full", redaction: { keys: ["secret"] }, client: durability,
  });
  ...
  const hidden = createPipelineInspectionClient({ cursorSigningKey: key, visibility: "hidden", client: durability });
