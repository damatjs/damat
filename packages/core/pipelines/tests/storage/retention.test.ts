...
test("retention deletes complete root trees and their pipeline-owned jobs", async () => {
  ...
  expect(await runPipelineRetention({ ...request(key), batchSize: 1 })).toEqual(
    result,
  );
  ...
  expect(
    await inspection.runRetention(request(`empty-${crypto.randomUUID()}`)),
  ).toEqual({
    deletedRuns: 0,
    deletedJobs: 0,
  });
...
test("retention validates dates, bounds, and audited controls", async () => {
  await expect(
    runPipelineRetention({
      ...request("date"),
      terminalBefore: new Date("invalid"),
    }),
  ).rejects.toThrow("valid date");
  for (const batchSize of [0, 1_001, 1.5]) {
    await expect(
      runPipelineRetention({ ...request(`batch-${batchSize}`), batchSize }),
    ).rejects.toThrow("between 1 and 1000");
  }
  await expect(
    runPipelineRetention({ ...request("reason"), reason: " " }),
  ).rejects.toThrow("reason");
  await expect(runPipelineRetention({ ...request(" ") })).rejects.toThrow(
    "idempotency",
  );
  await expect(
    runPipelineRetention({
