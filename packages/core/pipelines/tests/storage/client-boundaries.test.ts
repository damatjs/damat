...
test("start validates names, published versions, schemas, and executors", async () => {
  ...
  await expect(startPipeline(uniqueName("missing"), {})).rejects.toThrow(
    "was not found",
  );
  await expect(
    startPipeline("name", {}, { executor: durability as never }),
  ).rejects.toThrow("active transaction executor");
  ...
  await expect(startPipeline(name, {})).rejects.toThrow(
    `pipeline.${name}.input.id`,
  );
...
test("declared signals are durable, idempotent, and transaction-aware", async () => {
  ...
  const first = await signalPipelineRun(
    run.id,
    "approve",
    { value: 1 },
    signal("one"),
  );
  expect(await signalPipelineRun(run.id, "approve", null, signal("one"))).toBe(
    first,
  );
  await durability.transaction(async (executor) => {
    expect(
      await signalPipelineRun(
        ...
        "approve",
        {},
        signal("two", executor as never),
      ),
    ).toBeString();
  });
  await expect(
    signalPipelineRun(run.id, "other", {}, signal("other")),
  ...
  await expect(
    signalPipelineRun(
      ...
      "approve",
      {},
      signal("bad", durability as never),
    ),
  ).rejects.toThrow("active transaction executor");
  await routePipelineCycle(100);
  await expect(
    signalPipelineRun(run.id, "approve", {}, signal("late")),
