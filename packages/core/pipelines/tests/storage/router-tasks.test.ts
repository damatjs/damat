...
test("router dispatches action, job, and workflow tasks with retry policy", async () => {
  ...
  registerPipelineJob({ name: job, inputSchema: { type: "number" }, outputSchema: { type: "number" } });
  registerPipelineWorkflow({ name: workflow } as never);
  const definition = definePipeline(uniqueName("tasks"), {
    version: 1,
    start: "action",
    nodes: [
      { id: "action", kind: "action", name: action, input: 1,
    ...
    edges: [{ from: "action", to: "job" }, { from: "job", to: "workflow" }],
  });
  await syncPipelineDefinitions();
  const run = await startPipeline(definition.name, {}, { correlationId: "correlation" });
...
test("terminal job projection fails invalid outputs and terminal job states", async () => {
  ...
  expect((await routeRunWithJobs(invalid.id, "succeeded", "bad")).status).toBe("failed");
  const dead = await startPipeline(definition.name, {}, { idempotencyKey: crypto.randomUUID() });
  expect((await routeRunWithJobs(dead.id, "dead_lettered")).status).toBe("failed");
  const cancelled = await startPipeline(definition.name, {}, { idempotencyKey: crypto.randomUUID() });
  expect((await routeRunWithJobs(cancelled.id, "cancelled")).status).toBe("failed");
});
