...
const child = () => definePipeline(uniqueName("foreach-child"), {
  version: 1, start: "wait", nodes: [{ id: "wait", kind: "delay" as const, delayMs: 0 }], edges: [],
});
const parent = (pipeline: string, items: unknown, maxItems = 3) => definePipeline(uniqueName("foreach-parent"), {
  version: 1,
  start: "each",
  nodes: [{ id: "each", kind: "foreach", pipeline, items: items as never, maxItems, concurrency: 1 }],
  edges: [],
});
...
test("foreach handles empty, invalid, oversized, and bounded item sets", async () => {
  ...
  expect((await routeToTerminal((await startPipeline(empty.name, {})).id)).status).toBe("succeeded");
  expect((await routeToTerminal((await startPipeline(invalid.name, {})).id)).status).toBe("failed");
  expect((await routeToTerminal((await startPipeline(oversized.name, {})).id)).status).toBe("failed");
  expect((await routeToTerminal((await startPipeline(bounded.name, {})).id)).status).toBe("succeeded");
  expect(storedItems({ input: { items: [1] } } as never)).toEqual([1]);
  expect(() => storedItems({ node_id: "each", input: null } as never)).toThrow("no stored item");
});
