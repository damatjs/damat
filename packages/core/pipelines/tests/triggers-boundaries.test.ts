...
test("cron validation rejects malformed fields and impossible schedules", () => {
  expect(() => nextCronOccurrence("* * * *", new Date())).toThrow(
    "five UTC fields",
  );
  ...
  expect(() =>
    nextCronOccurrence("0 0 31 2 *", new Date("2026-01-01Z")),
  ).toThrow("within two years");
});
      ...
      rows: [
        {
          ...
          manifest: {
            start: "node",
            nodes: [{ id: "node", kind: "delay", delayMs: 0 }],
            edges: [],
          },
        },
      ],
...
test("trigger receipts report insert and replay outcomes", async () => {
  for (const rowCount of [1, 0]) {
    ...
    expect(
      await recordTriggerReceipt(executor as never, "v", "t", "s", "r"),
    ).toBe(rowCount === 1);
  }
});
