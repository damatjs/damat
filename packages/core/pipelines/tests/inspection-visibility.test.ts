...
test("full and hidden manifests expose the intended authoring fields", () => {
  const manifest: PipelineManifest = {
    start: "wait",
    nodes: [
      { id: "wait", kind: "signal.wait", signal: "go", input: { secret: 1 } },
    ],
    edges: [],
    triggers: [
      { id: "tick", kind: "interval", everyMs: 10, input: { secret: 1 } },
    ],
    inspection: { visibility: "hidden" },
  };
  expect(visibleManifest(manifest, options("full"))).toMatchObject({
    start: "wait",
  });
  const hidden = visibleManifest(manifest, options("hidden"));
  expect(hidden.nodes).toEqual([
    { id: "wait", kind: "signal.wait", signal: "go" },
  ]);
  expect(hidden.triggers).toEqual([
    { id: "tick", kind: "interval", everyMs: 10 },
  ]);
  expect(
    inspectionOptionsForManifest(options("full"), manifest).visibility,
  ).toBe("hidden");
  expect(
    inspectionOptionsForManifest(options("metadata"), {
      ...manifest,
...
test("run and node visibility handles values, metadata, and errors", () => {
  ...
  expect(visibleRun(run, options("hidden"))).toMatchObject({
    metadata: {},
    trigger: {},
    error: { name: "Boom" },
  });
