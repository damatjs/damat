...
const execute = (capability: string, input: unknown) =>
  getJobDefinition(PIPELINE_EXECUTOR_JOB)!.handler(payload(capability, input), context);
...
test("pipeline executor invokes registered durable workflows", async () => {
  ...
  } as never, { inputSchema: { type: "number" }, outputSchema: { type: "number" } });
  expect(await execute(name, 3)).toBe(4);
  expect(calls).toHaveLength(1);
  await expect(execute(name, "bad")).rejects.toThrow(`${name}.input`);
  await expect(execute(`missing-${crypto.randomUUID()}`, 1)).rejects.toThrow("Unknown pipeline workflow");
});
...
test("pipeline executor propagates workflow failure and invalid output", async () => {
  ...
  registerPipelineWorkflow({
    name: failed,
    execute: async () => ({ success: false, error: new Error("workflow failed") }),
  } as never);
  ...
  } as never, { outputSchema: { type: "number" } });
