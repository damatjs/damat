...
import type { GenerateTypesOptions, GeneratedFilesMap } from "@/types";
...
export function generateFilesMap(
  ...
  for (const table of schema.tables) {
    ...
    result.set(
      fileName,
      generateTableFile(table, schema, autoFields, banner, logger),
    );
    ...
    result.set(zodFileName, generateZodFile(table, schema, banner, logger));
  }
