...
| `createDiffMigration(moduleName, moduleResolver, opts?, layout?)` | function     | Diff migration vs the saved snapshot; an optional layout selects a separate migrations directory.       |
...
| `runSystemMigrations`, `getSystemMigrationStatus`                 | functions    | Execute and inspect ordered inline system migrations through the shared tracker.                        |
...
`createMigration` and both direct builders take the module's own model resolver,
