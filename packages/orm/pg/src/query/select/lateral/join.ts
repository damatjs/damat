...
export function buildLateralJoin(
  ...
  if (opts.where) {
    for (const [col, val] of Object.entries(opts.where)) {
      if (targetCols.has(col)) {
        userWhereParts.push(
          compileRelCondition(
            `${quoteIdent(innerAlias)}.${quoteIdent(col)}`,
            val,
            params,
          ),
        );
  ...
  if (opts.whereRaw) {
    ...
    for (const raw of raws) {
      ...
      userWhereParts.push(
        raw.sql.replace(/\$(\d+)/g, (_, n) => `$${parseInt(n, 10) + offset}`),
      );
  ...
  const whereClause =
    allWhere.length > 0 ? `WHERE ${allWhere.join(" AND ")}` : "";
  const orderByClause =
    opts.orderBy && opts.orderBy.length > 0
      ? buildOrderByClause(
          opts.orderBy.map((o) => {
            const item: import("../../types").OrderByClause = {
