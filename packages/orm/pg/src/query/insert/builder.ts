...
export class InsertBuilder<Cols extends string = string> extends QueryBase<Cols> {
  ...
  onConflict(clause: OnConflictClause<Cols>): this {
    if (clause.conflictColumns) {
      this._assertColList(clause.conflictColumns as string[], "insert.onConflict.conflictColumns");
    }
    if (clause.action === "update" && clause.set) {
      this._assertCols(clause.set as Record<string, unknown>, "insert.onConflict.set");
    }
  ...
  generateSql(): BuiltQuery {
    if (this._rows.length === 0) {
      throw new Error("[query:insert] No values provided — call .values() before .generateSql()");
    }
    ...
    const conflictClause = this._onConflict ? buildOnConflictSql(this._onConflict as any, params) : "";
    const parts = [
  ...
  generateJson(): InsertDescriptor {
    ...
    if (this._onConflict) {
      ...
      desc.onConflict = {
        action: oc.action,
        ...(oc.conflictColumns ? { conflictColumns: [...oc.conflictColumns] } : {}),
