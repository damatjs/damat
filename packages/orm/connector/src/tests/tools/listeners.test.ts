...
describe("setupPoolListeners", () => {
    ...
    describe("event handlers", () => {
        it("should log debug on connect event", () => {
            pool.emit("connect");
            expect(logger.messages("debug")).toEqual(["New client connected to pool"]);
        });
        ...
        it("should log debug on release event", () => {
            pool.emit("release");
            expect(logger.messages("debug")).toEqual(["Client released back to pool"]);
        });
    ...
    describe("with a real Logger (spied)", () => {
        it("should call Logger.debug / Logger.error on the underlying methods", () => {
            ...
            expect(errorSpy).toHaveBeenCalledWith("PostgreSQL pool error", { error: "kaput" });
