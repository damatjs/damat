...
describe("module artifact resolution", () => {
  test("uses receiver mappings and CLI targets", async () => {
    ...
    const result = await resolveModuleInstall(ctx, "/source", {
      resolve: mock(async () => resolvedArtifact),
      origin,
      ports: mock(() => ({}) as never),
    ...
    expect(recipe.mock.calls[0]?.[0]).toHaveProperty(
      "receiver.name",
      "receiver",
    );
  ...
  test("accepts parsed origins and cleans up profile failures", async () => {
    ...
    const { ctx } = createContext({});
    await expect(
      resolveModuleInstall(ctx, request, {
        resolve: mock(async () => artifact(cleanup)),
        origin: mock(() => request),
        ports: mock(() => ({}) as never),
        loadProfile: mock(() => {
          throw new Error("bad");
        ...
        exists: () => false,
        readManifest: mock(() => manifest()),
        options: mock(() => ({})),
        recipe: mock(() => ({}) as never),
      }),
    ).rejects.toThrow("bad");
