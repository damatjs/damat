...
import { createInstallerPorts, createInstallerRuntime } from "../../installer/runtime";
...
describe("installer CLI adapters", () => {
  test("adapts fetch responses for artifact acquisition", async () => {
    ...
    globalThis.fetch = mock(async () => new Response('{"ok":true}', { status: 201 }));
    try {
      const response = await createInstallerPorts(context()).fetch!("https://example.test/a");
      expect(response.ok).toBe(true);
      expect(response.status).toBe(201);
      expect(await response.json()).toEqual({ ok: true });
      const bytes = await createInstallerPorts(context()).fetch!("https://example.test/a");
  ...
  test("maps runtime flags, package manager, and logger calls", () => {
    const ctx = context({
      "package-manager": "pnpm", "dry-run": true, "allow-scripts": true,
    });
    ...
    expect(runtime).toMatchObject({
      dryRun: true, allowScripts: true, packageManager: "pnpm",
    });
    ...
    expect(createInstallerRuntime(context({ "package-manager": "other" })).packageManager)
      .toBeUndefined();
  });
});
