...
| `@damatjs/framework/middleware` | module | `setupMiddleware`, `errorHandler`, `handleError`, `notFoundHandler`, `requestSetup`, `createRateLimitMiddleware`, `createAuthMiddleware`, `createValidatorMiddleware`, `validate`, `corsConfigSetter`, `getErrorCodeFromStatus`, and `CorsConfigType`. |
