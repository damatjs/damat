...
const rl = (requests: number, window = "1m"): HttpRateLimitConfig => ({ requests, window });

describe("resolveMethodConfig", () => {
  describe("rate limit precedence", () => {
    it("returns empty config when nothing is configured", () => {
      const result = resolveMethodConfig("GET", undefined, undefined, undefined, undefined);
      expect(result).toEqual({});
    });

    it("uses the global rate limit as a fallback and records it", () => {
      const global = rl(100, "1h");
      const result = resolveMethodConfig("GET", undefined, undefined, global, undefined);
    ...
    it("prefers the route-level rate limit over the global one", () => {
      ...
      const result = resolveMethodConfig("GET", route, undefined, rl(100), undefined);
    ...
    it("prefers the method-level rate limit over route and global", () => {
      ...
      const methods: RouteModuleConfig[] = [{ method: "POST", rateLimit: rl(5) }];
      const result = resolveMethodConfig("POST", route, methods, rl(100), undefined);
      expect(result.rateLimit).toEqual(rl(5));
    });

    it("explicitly disables the rate limit when method config sets it to false", () => {
      const route: RouteModuleConfig = { method: "POST", rateLimit: rl(10) };
      const methods: RouteModuleConfig[] = [{ method: "DELETE", rateLimit: false }];
      const result = resolveMethodConfig("DELETE", route, methods, rl(100), undefined);
    ...
    it("falls back to route/global for methods without a matching method config", () => {
      const methods: RouteModuleConfig[] = [{ method: "POST", rateLimit: rl(5) }];
      const result = resolveMethodConfig("GET", undefined, methods, rl(100), undefined);
  ...
  describe("auth precedence", () => {
    ...
    it("uses the global auth type as a fallback", () => {
      const result = resolveMethodConfig("GET", undefined, undefined, undefined, auth("session"));
    ...
    it("prefers route-level auth over global", () => {
      ...
      const result = resolveMethodConfig("GET", route, undefined, undefined, auth("session"));
      expect(result.auth).toEqual(auth("apiKey"));
    });

    it("prefers method-level auth over route and global", () => {
      const route: RouteModuleConfig = { method: "POST", auth: auth("apiKey") };
      const methods: RouteModuleConfig[] = [{ method: "PUT", auth: auth("flexible") }];
      const result = resolveMethodConfig("PUT", route, methods, undefined, auth("session"));
    ...
    it("explicitly disables auth when method config sets it to false", () => {
      ...
      const result = resolveMethodConfig("GET", route, methods, undefined, auth("session"));
