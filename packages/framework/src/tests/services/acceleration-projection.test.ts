...
test("projection rebuild replaces job, route, and delivery ready indexes", async () => {
  const now = new Date(1_000);
  const executor = {
    query: async (sql: string) => {
      if (sql.includes("_damat_job_runs")) return rows([{ id: "job", scope: "q", available_at: now }]);
      if (sql.includes("routed_at")) return rows([{ id: "event", scope: "router", available_at: now }]);
      return rows([{ id: "delivery", scope: '["mail","audit"]', available_at: now }]);
    },
  ...
  await rebuildReadyProjection(redis, { jobs: true, events: true }, executor);
...
test("projection audit writes actor, status, and details", async () => {
  ...
  await auditAccelerationRebuild(actor, "failed", { error: "offline" }, executor);
  expect(queries.map(({ params }) => params?.[0])).toEqual(["requested", "failed"]);
